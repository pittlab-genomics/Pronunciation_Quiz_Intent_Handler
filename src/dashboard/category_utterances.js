...
exports.list_handler = async function (event) {
    console.log(`[dashboard.category_utterances_list] event: ${JSON.stringify(event)}`);
    let response = {};
    let status_code = 200;
    try {
        const all_utterances = await utterances_repository.getAllCategoryUtterances();
        response = { "data": {
        ...
        status_code = 500;
        console.error(`[dashboard.category_utterances_list] Exception raised: ${JSON.stringify(err)}`, err);
...
exports.stats_handler = async function (event) {
    ...
    try {
        ...
        const category_utterances_stats = await utterances_repository.getUtterancesCountByCategory(params);
        response = { "data": category_utterances_stats };
    }
    catch (err) {
        status_code = 500;
        console.error(`[dashboard.category_utterances_list] Exception raised: ${JSON.stringify(err)}`, err);
